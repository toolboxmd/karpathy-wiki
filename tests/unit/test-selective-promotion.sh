...
python3 "${CONFIG}" route-set --workspace "${PROJECT}" --mode both \
  --project-wiki "${PROJECT}" --main-wiki "${MAIN}" >/dev/null

set_project_mode() {
  local selected="$1"
  case "${selected}" in
    project)
      python3 "${CONFIG}" route-set --workspace "${PROJECT}" --mode project \
        --project-wiki "${PROJECT}" >/dev/null
      ;;
    both)
      python3 "${CONFIG}" route-set --workspace "${PROJECT}" --mode both \
        --project-wiki "${PROJECT}" --main-wiki "${MAIN}" >/dev/null
...
assert pin["source_capture_name"] == "reusable.md"
assert len(pin["source_fingerprint"]) == 64
...
test_no_cross_workspace_pin_reuse() {
  ...
  python3 "${CONFIG}" route-set --workspace "${other}" --mode both \
    --project-wiki "${other}" --main-wiki "${SECOND_MAIN}" >/dev/null
...
test_external_pin_lookup_survives_parent_pointer_removal() {
  ...
  python3 "${CONFIG}" route-set --workspace "${workspace}" --mode both \
    --project-wiki "${nested}" --main-wiki "${MAIN}" >/dev/null
...
test_publish_rechecks_live_workspace_mode() {
  local capture body before after pin_count
  capture="$(make_source stale-live-mode)"
  body="${TESTDIR}/stale-live-mode-body.md"
  make_body "${body}"
  before="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  pin_count="$(find "${XDG_CONFIG_HOME}/karpathy-wiki/promotions" -type f -name 'prom-*.json' 2>/dev/null | wc -l | tr -d ' ')"

  set_project_mode project
  run_promote "${capture}" publish --title "Must not publish" --body-file "${body}" >/dev/null 2>&1 \
    && fail "publish honored stale selective metadata after mode changed to project"
  after="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  [[ "${after}" -eq "${before}" ]] || fail "stale-mode publish changed the main queue"
  [[ "$(find "${XDG_CONFIG_HOME}/karpathy-wiki/promotions" -type f -name 'prom-*.json' 2>/dev/null | wc -l | tr -d ' ')" -eq "${pin_count}" ]] \
    || fail "stale-mode publish created a trusted pin"
  grep -q '^promotion_decision: null' "${capture}" \
    || fail "stale-mode publish changed the source decision"
  set_project_mode both
}

test_reused_capture_id_for_distinct_source_is_rejected() {
  local first second body first_target before after
  first="$(make_source reused-id-first)"
  second="$(make_source reused-id-second)"
  sed -i.bak 's/cap-reused-id-second/cap-reused-id-first/' "${second}"
  rm -f "${second}.bak"
  body="${TESTDIR}/reused-id-body.md"
  make_body "${body}"

  run_promote "${first}" publish --title "First source" --body-file "${body}"
  first_target="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' -exec grep -l '^propagated_from: "cap-reused-id-first"$' {} + | head -1)"
  [[ -n "${first_target}" ]] || fail "first reused-ID source did not publish"
  before="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  run_promote "${second}" publish --title "Second source" --body-file "${body}" >/dev/null 2>&1 \
    && fail "distinct source reused another capture ID"
  after="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  [[ "${after}" -eq "${before}" ]] || fail "reused source ID created another target"
  grep -q '^promotion_decision: null' "${second}" \
    || fail "reused source ID marked the second source terminal"
}

test_invalid_utf8_source_fails_cleanly() {
  local capture body output before after
  capture="$(make_source invalid-utf8)"
  printf '\377\376' >> "${capture}"
  body="${TESTDIR}/invalid-utf8-body.md"
  make_body "${body}"
  before="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  output="$(run_promote "${capture}" publish --title "Invalid UTF-8" --body-file "${body}" 2>&1 || true)"
  [[ -n "${output}" ]] || fail "invalid UTF-8 produced no controlled error"
  ! grep -q 'Traceback' <<< "${output}" || fail "invalid UTF-8 leaked a traceback"
  after="$(find "${MAIN}/.wiki-pending" -maxdepth 1 -type f -name '*prom-*.md' | wc -l | tr -d ' ')"
  [[ "${after}" -eq "${before}" ]] || fail "invalid UTF-8 changed the main queue"
}

test_spaced_yaml_keys_are_parsed_without_duplicates() {
  local capture
  capture="$(make_source spaced-key)"
  sed -i.bak \
    -e 's/^promotion_policy: /promotion_policy : /' \
    -e 's/^promotion_decision: /promotion_decision : /' \
    "${capture}"
  rm -f "${capture}.bak"
  run_promote "${capture}" keep-local \
    || fail "valid YAML spacing made selective capture ineligible"
  [[ "$(grep -Ec '^promotion_decision[[:space:]]*:' "${capture}")" -eq 1 ]] \
    || fail "spaced decision update created a duplicate key"
  grep -Eq '^promotion_decision[[:space:]]*: "keep-local"$' "${capture}" \
    || fail "spaced decision was not updated in place"
}
