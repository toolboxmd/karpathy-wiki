...
A generic tracing playbook would be too broad for these details. The facts
below are limited to Wicktrace commands and version 0.9 behavior.

Possible later work: a sampled flame export, `wicktrace flame --sample 0.01`,
which does not exist in 0.9. It would downsample stacks before writing a folded
file. Do not document it as current behavior. The current export is `wicktrace
dump --format jsonl` only.
