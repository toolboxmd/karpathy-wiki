...
sync_fresh_pointer_target() {
  local enabled="$1" target="$(pwd)/wiki" previous_fork

  # Workspace-bound trust validation requires the candidate pointer, but the
  # pointer is not committed until after the existing runtime is synchronized.
  if ! write_project_pointer_config "${enabled}"; then
  ...
  previous_fork="$(python3 "${CONFIG_TOOL}" get \
    --wiki "${target}" --key routing.fork_to_main 2>/dev/null)" || return 1
  set_runtime_fork "${target}" "${enabled}" || return 1
  if ! write_project_pointer_config "${enabled}"; then
    rm -f "$(pwd)/.wiki-config"
...
case "${mode}" in
  project)
    ...
    elif ! sync_fresh_pointer_target false; then
      echo >&2 "wiki use project: existing ./wiki is not configured and trusted for this workspace, or its routing could not be synchronized"
      exit 1
    fi
    rm -f "$(pwd)/.wiki-mode"
    echo "wiki use project: $(pwd)/wiki/ (fork_to_main = false)"
...
EOF
    ...
    if [[ -f "$(pwd)/.wiki-config" ]]; then
      ...
      if [[ ! -d "$(pwd)/wiki" ]]; then
        ...
        if ! sync_fresh_pointer_target true; then
          echo >&2 "wiki use both: existing ./wiki is not configured and trusted for this workspace, or its routing could not be synchronized"
          exit 1
        fi
        rm -f "$(pwd)/.wiki-mode"
