...
def _source_identity(capture: Path) -> tuple[str, str]:
    """Bind a promotion pin to one logical source, not only its portable ID."""

    name = capture.name.removesuffix(".processing")
    text = capture.read_text(encoding="utf-8")
    lines, closing = _frontmatter(text)
    mutable = {"promotion_decision", "promotion_id", "promotion_main_wiki"}
    stable = [lines[0]]
    for line in lines[1:closing]:
        key = line.split(":", 1)[0].strip()
        if key not in mutable:
            stable.append(line)
    stable.extend(lines[closing:])
    normalized = "".join(stable).replace("\r\n", "\n")
    fingerprint = hashlib.sha256(
        name.encode("utf-8") + b"\0" + normalized.encode("utf-8")
    ).hexdigest()
...
def _read_pin(
    root: Path, capture: Path, capture_id: str, promotion_id: str
) -> dict[str, Any] | None:
    ...
    pin = _read_json(path)
    source_name, source_fingerprint = _source_identity(capture)
    expected = {
        "schema_version": 2,
...
def _live_both_route(root: Path, plugin: Path) -> tuple[Path, Path]:
    """Return the trusted workspace and exact main target for a new intent."""

    command = [
        sys.executable,
        str(plugin / "scripts" / "wiki_config.py"),
        "route-find",
        "--cwd",
        str(root),
        "--json",
    ...
    if result.returncode != 0:
        detail = result.stderr.strip() or "workspace routing is unavailable"
        raise PromotionError(f"cannot authorize selective promotion: {detail}")
    try:
        route = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise PromotionError("workspace routing returned invalid JSON") from exc
    if route.get("mode") != "both":
        raise PromotionError("current workspace mode is not both")
    if Path(str(route.get("project_wiki", ""))).resolve() != root:
        raise PromotionError("workspace routing does not target this project wiki")
    workspace = Path(str(route.get("workspace_root", ""))).resolve()
    main_value = route.get("main_wiki")
    if not isinstance(main_value, str) or not main_value:
        raise PromotionError("both mode has no exact main wiki target")
    return workspace, _validate_main_wiki(Path(main_value))
...
def keep_local() -> int:
    ...
    with (state_dir / f"{promotion_id}.lock").open("a+", encoding="utf-8") as lock:
        ...
        if _read_pin(root, capture, capture_id, promotion_id) is not None:
...
def verify_decision() -> int:
    ...
    with (state_dir / f"{promotion_id}.lock").open("a+", encoding="utf-8") as lock:
        ...
        if decision == "keep-local":
            ...
            if receipt_path.exists() or _read_pin(root, capture, capture_id, promotion_id) is not None:
        ...
        pin = _read_pin(root, capture, capture_id, promotion_id)
        if pin is None:
...
def publish(title: str, body_file: str) -> int:
    ...
    with lock_path.open("a+", encoding="utf-8") as lock:
        ...
        pin_path, fallback_workspace = _pin_path(root, promotion_id)
        pin = _read_pin(root, capture, capture_id, promotion_id)
        if pin is None:
            workspace, main = _live_both_route(root, plugin)
            ...
            pin = {
                "schema_version": 2,
                "source_capture_id": capture_id,
                "source_capture_name": _source_identity(capture)[0],
                "source_fingerprint": _source_identity(capture)[1],
                "promotion_id": promotion_id,
                "canonical_project_wiki": str(root),
                "canonical_workspace": str(workspace or fallback_workspace),
